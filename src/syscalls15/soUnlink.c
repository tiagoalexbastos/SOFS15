 ...
 *  \return -<em>other specific stator</em> issued by \e lseek system call
 */
...
int soUnlink (const char *ePath)
{
  ...
  if((stat = soGetDirEntryByPath(ePath, &nInodeDir, &nInodeEnt)) != 0)
      return stat;
 ...
 if((inodeDir.mode & INODE_DIR)!= INODE_DIR)
   ...
   if((inodeEnt.mode & INODE_DIR)== INODE_DIR)
      return -EISDIR;
  if((stat = soAccessGranted(&inodeEnt,W)) != 0){
		return stat;
	}

   
   if((stat = soRemDetachDirEntry(nInodeDir,base,REM)) != 0)
      return stat;


  return 0;
}
//-\c ELOOP, if the path resolves to more than one symbolic link